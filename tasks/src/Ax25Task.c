...
portTASK_FUNCTION_PROTO(Ax25Task, pvParameters)
{
    BaseType_t xStatus;
    int chan;
    AX25_data_link_state_machine_t *state;
...
//    printf("Initializing Ax25 Data Link Task\n");
    ...
    for (chan = 0; chan < NUM_RX_CHANNELS; chan++) {
        ...
        xIFrameQueue[chan] = xQueueCreate(IFRAME_QUEUE_LEN,
                                          sizeof(AX25_event_t));
    ...
    while (1) {
        ReportToWatchdog(Ax25TaskWD);
        xStatus = xQueueReceive(xRxPacketQueue, &ax25_radio_buffer,
                                CENTISECONDS(1) );
        if (xStatus == pdPASS) {
            /* Data was successfully received from the queue */
            /* LM_DATA_Indicate - Frames of any type passed from the
               Link Multiplexer to the Data Link State Machine */
            ax25_process_lm_frame(ax25_radio_buffer.channel);
        }
        ReportToWatchdog(Ax25TaskWD);
        /* Yield some time so any packets are sent, or so that others
           may do some processing */
//        vTaskDelay(CENTISECONDS(1));
        taskYIELD();

        ReportToWatchdog(Ax25TaskWD);
        xStatus = xQueueReceive(xRxEventQueue, &ax25_received_event,
                                CENTISECONDS(1) );
        if (xStatus == pdPASS) {
            chan = ax25_received_event.rx_channel;

            if (chan >= NUM_RX_CHANNELS) {
                // something is seriously wrong.  Programming error.
                // Unlikely to occur in flight
                debug_print("ERR: AX25 channel %d is invalid\n",
                            ax25_received_event.rx_channel);
            } else {
                state = &data_link_state_machine[chan];
                if (ax25_received_event.primitive == LM_SEIZE_Request) {
                    trace_dl("LM[%d]: SEIZE Requested\n",
                             ax25_received_event.rx_channel);
                    seize_requested[chan] = TRUE;
                } else {
                    ax25_next_state_from_primitive(state, &ax25_received_event);
                }
            }
        }
        ReportToWatchdog(Ax25TaskWD);
        ...
        if (!in_test) {
            /*
             * See if any channels have I frames to send.  Send them
             * now, which will also process any pending
             * acknowledgement.  If SEIZE Request is set then send the
             * DL State Machine a SEIZE Confirm message after any
             * frames are sent.  This will cause an RR to be sent if
             * an ACK is still pending.
             */
            for (chan = 0; chan < NUM_RX_CHANNELS; chan++) {
                state = &data_link_state_machine[chan];
                xStatus = xQueueReceive(xIFrameQueue[chan], &ax25_received_event,
                                        CENTISECONDS(1));
                if (xStatus == pdPASS) {
//#ifdef DEBUG
...
//#endif
                ...
                taskYIELD();
                ReportToWatchdog(Ax25TaskWD);
                if (seize_requested[chan]) {
                    lm_event.primitive = LM_SEIZE_Confirm;
                    ax25_next_state_from_primitive(state, &lm_event);
                    seize_requested[chan] = false;
                }
            }
        }
    }
 ...
 * ax25_send_status()
 *
 * This is called from the Telemetry and Control task to send the
 * status periodically Puts a packet with the current status of the
 * Uplink into the TxQueue
 *
...
void ax25_send_status() {
    ...
    if (!ReadMRAMBoolState(StateUplinkEnabled)) {
        ...
        char buffer[25];
        /*
         * Open ABCD.  - we do not send the string termination char or
         * it is transmitted too
         */
        uint8_t len = 6 + NUM_RX_CHANNELS;
        ...
        for (chan = 0; chan < NUM_RX_CHANNELS; chan++) {
            if (rx_working(chan) &&
                      data_link_state_machine[chan].dl_state == DISCONNECTED) {
                strlcat(buffer, rx_channel_names[chan], sizeof(buffer));
            } else {
                channels_available--;
...
void ax25_t1_expired(TimerHandle_t xTimer)
{
    ...
    // Do not block as this is called from timer
...
#ifdef DEBUG
    // timer id is treated as an integer and not as a pointer
    uint32_t chan = (uint32_t)pvTimerGetTimerID( timer );
#endif
 ...
 *
 * Process an LM_DATA_indicate message, which is an AX25 frame
 * received from the radios.  If the state machine for the given
 * channel is inactive then we process the data.  If the frame
 * received is not for the callsign connected on this channel then we
 * ignore it.  This is called from the main processing loop whenever a
 * frame is received.  We decode the bytes, which stores a copy of the
...
void ax25_process_lm_frame(uint8_t channel)
{
    char *from_callsign;
    char *to_callsign;
    AX25_data_link_state_machine_t *state = &data_link_state_machine[channel];
    ...
    if (strcasecmp(to_callsign, BBS_CALLSIGN) == 0) {
        if (state->dl_state == DISCONNECTED) {
            /* New connection. */
            strlcpy(state->callsign, from_callsign, MAX_CALLSIGN_LEN);
            state->channel = channel;
            ax25_next_state_from_packet(state, dp);
        } else {
            /*
             * This needs to be the same callsign for this channel,
             * otherwise we are busy.
             */
            if (strcasecmp(state->callsign, from_callsign) == 0) {
                ax25_next_state_from_packet(state, dp);
            } else {
        ...
        if (dp->via_callsign[0] != 0) {
             ...
             *
             * We simplify things by only allowing one via digi, so it
             * needs to be the first one.
             */
            if (strcasecmp(dp->via_callsign, DIGI_CALLSIGN) == 0 &&
                    !dp->via_h) {
                //debug_print(".....VIA Digi \n");
                // Set the repeated bit, which is the last byte of the
                // third callsign ie bit 7 byte 21
 ...
 *
 * The dl_state of PF should already be set already in the response
 * packet If NR needs to be set, then that should already be set too.
 * If a packet can only be a COMMAND or RESPONSE then the TX will send
 * it correctly.  If there is a choice then it should already be set
 * correctly in the packet
 */
void ax25_send_response(uint8_t channel, ax25_frame_type_t frame_type,
                        char *to_callsign, AX25_PACKET *response_packet,
                        bool expedited)
{
    ...
    if (rc == FALSE) {
        ...
        ReportError(TxPacketDropped, FALSE, CharString,
                    (int)"TX Send Falure.  Packet Dropped.");
 ...
 */
bool ax25_send_event(AX25_data_link_state_machine_t *state,
                     AX25_primitive_t prim, AX25_PACKET *packet,
...
/**
 * Send event to Link Multiplexer, which uses the same event queue as
 * the Data Link state machines.  packet can be NULL if the event does
 * not pass a packet Sets error to NO_ERROR
 *
 */
bool ax25_send_lm_event(AX25_data_link_state_machine_t *state,
                        AX25_primitive_t prim)
{
    BaseType_t xStatus;
...
void ax25_next_state_from_packet(AX25_data_link_state_machine_t *state,
                                 AX25_PACKET *decoded_packet)
{
    trace_dl("AX25[%d] **STATE (Packet)** %s VA=%d VS=%d VR=%d RC=%d ack_pend=%d\n",
             state->channel, state_names[state->dl_state], state->VA, state->VS,
             state->VR, state->RC, state->achnowledge_pending);
...
void ax25_next_state_from_primitive(AX25_data_link_state_machine_t *state,
                                    AX25_event_t *event)
{
 ...
 *
 * The next two functions process AX25 Primitives.  All primitives are
 * processed in the first function except LM_DATA_Indicate primitives,
 * which contain packets. They are all processed in the second
 * function, which takes the received packet as a parameter.
 *
 * The split between Primitives and Packets is repeated for each state.
 *
 */
void ax25_state_disc_prim(AX25_data_link_state_machine_t *state,
                          AX25_event_t *event)
{
...
void ax25_state_disc_packet(AX25_data_link_state_machine_t *state,
                            AX25_PACKET *packet)
{
 ...
 */
void ax25_state_wait_conn_prim(AX25_data_link_state_machine_t *state,
                               AX25_event_t *event)
{
    //trace_dl("AX25: STATE WAIT CONNECTION (prim): ");
    switch (event->primitive) {
        case DL_DISCONNECT_Request : {
            /* The spec says requeue the request and remain in the
             * same state.  I think the logic here is that we should
             * handle the request after we know if the other station
             * has connected to us or timed out.  This is a request
             * from Layer 3 to Disconnect while we are waiting for the
             * response to a connection request.  So an internal
             * error, a command from the ground or a timout has caused
             * the disconnect.  The other end has not heard our SABM
             * or it is trying to respond with its UA and we have not
             * heard it yet.  So they are in DISCONNECTED or CONNECTED
             * state.  This should follow the same logic as
             * DL_DISCONNECT_Request in the CONNECTED state.  If in
             * DISCONNECTED state we just send the Confirm Event to
             * Layer 3.  If we want to action this now, then we can
             * follow the logic for CONNECTED state.
             */
        ...
        case DL_DATA_Request : {
            ...
            /* We don't add the Iframe through this event.  Instead
             * the Uplink (Layer 3) adds it directly to the
             * IFrameQueue.  Then it will know if the Queue is full
             * and be able to throttle data */
        ...
        case DL_POP_IFRAME_Request : {
            ...
            if (state->layer_3_initiated_the_request) {
                // TODO - This is what the spec says to do, but we
                // will get stuck in a loop unless a new frame
                // received or we time out?
                
                // Instead we could ignore the data and tell layer 3
                // we are disconnected.  i.e. reset it.
...
void ax25_state_wait_conn_packet(AX25_data_link_state_machine_t *state,
                                 AX25_PACKET *packet)
{
    ...
    switch (packet->frame_type) {
        ...
        case TYPE_U_SABME : {
            ...
            ax25_send_response(state->channel, TYPE_U_FRMR, state->callsign,
                               &state->response_packet, EXPEDITED);
            // TODO - stay in wait connection and hope they send SABM
            // or move to DISCONNECTED?
            break;
        }
        case TYPE_U_DISC : {
            trace_dl("DISC\n");
        ...
        case TYPE_U_UA : {
            ...
            if (packet->PF == 1) {
                if (state->layer_3_initiated_the_request) {
                    // This is the only time we send CONNECT_Confirm,
                    // because Layer 3 requested the connection
                    ax25_send_event(state, DL_CONNECT_Confirm, packet, NO_ERROR);
                } else {
                    if (state->VS != state->VA) {
                        // this was in the original spec and seems to make sense
                        discard_iframe_queue(state);
                        ax25_send_event(state, DL_CONNECT_Indicate, packet,
                                        NO_ERROR);
                    }
                }
                stop_timer(timerT1[state->channel]);
 ...
 */
void ax25_state_wait_release_prim(AX25_data_link_state_machine_t *state,
                                  AX25_event_t *event)
{
...
void ax25_state_wait_release_packet(AX25_data_link_state_machine_t *state,
                                    AX25_PACKET *packet)
{
 ...
 */
void ax25_state_connected_prim(AX25_data_link_state_machine_t *state,
                               AX25_event_t *event)
{
...
void ax25_state_connected_packet(AX25_data_link_state_machine_t *state,
                                 AX25_PACKET *packet)
{
    ...
    switch (packet->frame_type) {
        case TYPE_U_SABM : {
            ...
            // Send ERROR Messsage to Layer 3.  We are already
            // connected.  Layer 3 should interpret this as a data
            // link reset
        ...
        case TYPE_U_DISC : {
            ...
            stop_timer(timerT3[state->channel]);
            // This is set to STOP as per the previous flow chart and DireWolf
            stop_timer(timerT1[state->channel]);
            state->dl_state = DISCONNECTED;
            break;
        }
 ...
 */
void ax25_state_timer_rec_prim(AX25_data_link_state_machine_t *state,
                               AX25_event_t *event)
{
    ...
    switch (event->primitive) {
       ...
       case DL_TIMER_T1_Expire : {
            trace_dl("Timer T1 Expired\n");
            if (state->RC == AX25_RETRIES_N2) {
                // MAX Retries reached, disconnect, just work out the
                // error first
                if (state->VA == state->VS) {
                    if (state->peer_receiver_busy) {
                        ax25_send_event(state, DL_ERROR_Indicate,
                                        &event->packet, ERROR_U);
                    } else {
                        ax25_send_event(state, DL_ERROR_Indicate,
                                        &event->packet, ERROR_T);
                    }
                } else {
                    ax25_send_event(state, DL_ERROR_Indicate, &event->packet,
                                    ERROR_I);
                }
                ax25_send_event(state, DL_DISCONNECT_Indicate, &event->packet,
...
void ax25_state_timer_rec_packet(AX25_data_link_state_machine_t *state,
                                 AX25_PACKET *packet)
{
    ...
    switch (packet->frame_type) {
        case TYPE_U_UA : {
            ...
            // Send ERROR Messsage to Layer 3.  We are already
            // connected.  Layer 3 can ignore this
            ax25_send_event(state, DL_ERROR_Indicate, NULL, ERROR_C);
            establish_data_link(state);
            clear_layer_3_initiated(state);
            state->dl_state = AWAITING_CONNECTION;
        ...
        // LM SIEZE CONFIRM - Implement if needed.  If we are able to
        // transmit then send Eqnuiry Response if AckPending...
 ...
 */
void connected_rframe_response(AX25_data_link_state_machine_t *state,
                               AX25_PACKET *packet)
{
    check_need_for_response(state, packet);
    if (VA_lte_NR_lte_VS(state, packet->NR)) {
 ...
 */
void timer_rec_rframe_response(AX25_data_link_state_machine_t *state,
                               AX25_PACKET *packet)
{
    check_need_for_response(state, packet);
    if (packet->command == AX25_RESPONSE && packet->PF == 1) {
...
/**
 * Called from the connected and timer_recovery states when the event
 * is received indicating that an I-Frame is available to be
 * transmitted and has been removed from the I-Frame queue
 */
void iframe_pops_off_queue(AX25_data_link_state_machine_t *state,
                           AX25_event_t *event)
{
    BaseType_t xStatus;
    bool rc;
    ...
    if (state->peer_receiver_busy) {
        ...
        state->achnowledge_pending = false;
        /*
         * The spec says that if T1 is running we do not need to start
         * T1 and stop T3.  DireWolf ignores that and always starts T1
         * and stops T3.  This prevents us timing out too soon.  That
         * logic seems to make sense so it is implemented here.
         */
 ...
 */
void process_iframe(AX25_data_link_state_machine_t *state, AX25_PACKET *packet,
                    AX25_data_link_state_t final_state)
{
#ifdef TRACE_AX25
            ...
            if (VA_lte_NR_lte_VS(state, packet->NR)) {
                ...
                if (state->own_receiver_busy) {
                    ...
                    if (packet->PF == 1) {
                        ...
                        ax25_send_response(state->channel, TYPE_S_RNR,
                                           state->callsign,
                    ...
                    if (packet->NS == state->VR) {
                        // We expect frame number VR and that is what we got
                        ...
                        ax25_send_event(state, DL_DATA_Indicate, packet,
                                        NO_ERROR);
                        ax25_copy_packet(packet,
                                         &state->I_frames_sent[packet->NS]);
                        ...
                        if (packet->PF == 1) {
                            ...
                            if (!state->achnowledge_pending) {
                                /* LM Seize Request
                                 *
                                 * This requests transmission at the
                                 * next possible opportunity.  The
                                 * ack_pending will go in the next
                                 * available packet, either an IFrame
                                 * or a dedicated RR Frame.
                                 */
                                state->achnowledge_pending = true;
                        ...
                        if (state->reject_exception) {
                            // We already have a rejection
                            // outstanding, discard and ignore iframe
                            if (packet->PF == 1) {
                                send_rr_frame(state,packet);
                            }
                            state->dl_state = final_state;
                            ...
                            // if srej was enabled, then we do that,
                            // but it is not
                            ...
                            state->response_packet.command = AX25_RESPONSE;
                            state->response_packet.NR = state->VR;
                            ax25_send_response(state->channel, TYPE_S_REJ,
                                               state->callsign,
                                               &state->response_packet,
                                               NOT_EXPEDITED);
                            state->achnowledge_pending = false;
...
/**
 * Send an RR frame when processing and I frame.  Called from the
 * process I frame function which is called from the Connected and
...
void send_rr_frame(AX25_data_link_state_machine_t *state, AX25_PACKET *packet)
{
    ...
    state->response_packet.command = AX25_RESPONSE;
    state->response_packet.NR = state->VR;
    ax25_send_response(state->channel, TYPE_S_RR, state->callsign,
                       &state->response_packet, NOT_EXPEDITED);
    state->achnowledge_pending = false;
}
 ...
 */
void nr_error_recovery(AX25_data_link_state_machine_t *state,
                       AX25_PACKET *packet)
{
...
void transmit_enquiry(AX25_data_link_state_machine_t *state)
{
    ...
    state->response_packet.NR = state->VR;
    if (state->own_receiver_busy) {
        ax25_send_response(state->channel, TYPE_S_RNR, state->callsign,
                           &state->response_packet, NOT_EXPEDITED);
    } else {
        ax25_send_response(state->channel, TYPE_S_RR, state->callsign,
                           &state->response_packet, NOT_EXPEDITED);
...
/**
 * This sends an RR or RNR frame in a couple of situations, If the
 * other end sent a packet with the POLL/FINAL bit set to 1 then they
 * want to know if we are still alive.  We respond with an RR frame
 * with the F bit set to 1, unless our receiver is busy, then we send
 * an RNR frame.
 *
 * This is also called when we simply want to send an ACK for packets
 * received so far.  It sends an RR frame with NR set to VR and the F
 * bit set to 0.
 */
                      ...
                      AX25_PACKET *packet, int F)
{
    ...
    if (packet != NULL && packet->PF == 1 && (packet->frame_type == TYPE_S_RR ||
        ...
        if (state->own_receiver_busy) {
            ...
            // SREJ is not enabled, otherwise additional logic needed
            // here for out of sequence frames and SREJ
 ...
 *
 * If we have 3 frames and VS = 6, for example, so we have sent 3,4,5
 * and a frame is in the queue, which will be 6 when we transmit it.
 * We have a reject with NR 4, which means 3 is accepted but 4 and 5
 * are not.
 *
 * We need to actually decrement VS because the frames are not sent
 * right now.  They are sent as they get popped off the Iframe queue.
 * When the first one is popped off it needs to have VS = 4 in this
 * case.  The frames in the queue don't store their VS number.  This
 * is set when it pops off the queue, but we need to get them in the
 * right order so they end up with the right VS numbers again, which
 * is set as NS in the frame.
 *
 * We can push frames onto the head of the queue so that they will be
 * sent before the frame that is sitting there now, so that will still
 * be sent with NS=6.  Note that 6 is in the queue (even though it is
 * not labeled yet).  We want to put 4 and 5 on the queue, but we must
...
void invoke_retransmission(AX25_data_link_state_machine_t *state, int NR)
{
    ...
    //backtrack.  Put all the frames on the queue again from NR.
    //Everything before is confirmed and VA already set to NR.
    
    //We need to change VS.  VS is the next frame we were going to
    //send. So it now needs to equal NR, it must currently be in front
    trace_dl("BACKTRACK: VS: %d to NEW NR:%d\n",state->VS, NR);
    vs = state->VS; // VS points to next frame we will send.
    do {
        // Start with the previous frame sent and go backwards
        vs = AX25_MODULO(vs - 1);
...
/**
 * We have received a frame with an NR that is an ack for all frames
 * up to that point.  Or rather it says "I am ready for the next
 * number NR", acknowledging through NR -1.  We set VA equal to that
 * NR number.
 */
void check_iframes_acknowledged(AX25_data_link_state_machine_t *state,
                                AX25_PACKET *packet)
{
    if (state->peer_receiver_busy) {
        state->VA = packet->NR;
        // revised flow chart says STOP T3. But PSGS and direwold have
        // Start T3 per the old flow chart.
        start_timer(timerT3[state->channel]);
        BaseType_t act = xTimerIsTimerActive(timerT1[state->channel]);
         if (act != pdPASS) {
             start_timer(timerT1[state->channel]);
        ...
        if (packet->NR == state->VS) {
            // essentially this is an RR and NR is the next frame for
            // us - normal situation
            state->VA = packet->NR;
            stop_timer(timerT1[state->channel]);
            start_timer(timerT3[state->channel]);
            ...
            if (packet->NR != state->VA) {
                // we got an ack for part of the frames we have sent,
                // but not all.  Set the ack variable VA to the number
                // we received.  Start T1 as we don't want to send any
                // more data yet.
                state->VA = packet->NR;
                start_timer(timerT1[state->channel]);
            }
            // otherwise the RR had an NR equal to the VA we already
            // have.  A second confirmation of where we are but if our
            // VS is out in front, don't we need to resend the frame?
            // What if VS > NR -
        }
    }
}
...
/**
 * Discard the queue of future Iframes to send.  This typically
 * happens if we reset the data link
 */
void discard_iframe_queue(AX25_data_link_state_machine_t *state)
{
    BaseType_t xStatus;
    ...
    if(xStatus != pdPASS) {
        /* The reset operation could not complete because something is
         * blocking the queue.  It does not look like this can
         * actually happen.  The underlying function only ever returns
         * pdPASS.  If this error message shows up in testing we can
         * work out how to handle it. */
        debug_print("AX25: SERIOUS ERROR: Could not reset the IFrame Queue\n");
    }
}

void check_need_for_response(AX25_data_link_state_machine_t *state,
                             AX25_PACKET *packet)
{
    if (packet->command == AX25_COMMAND && packet->PF == 1) {
        enquiry_response(state, packet, 1);
 ...
 * select_t1_value()
 *
 * This calculates the value in RTOS Ticks.  However, it is currently
 * not used.  A dynamic time for T1 is used in AX25 to cope with
 * Digipeters in the path.  That is not currently in scope for
 * PACSAT. If we decide that digipeter paths via one or more
 * spacecraft will be supported in some way, then this may need to be
 * implemented.
 *
 * Here are implementation notes from DireWolf.  This is not adopted
 * here, but these notes are left in case we implement in the future.
 * They come from this file:
 * https://github.com/wb2osz/direwolf/blob/master/src/ax25_link.c
 *
 * WB2OSZ says:
 *
 * It seems like a good idea to adapt to channel conditions, such as
 * digipeater delays, but it is fraught with peril if you are not
 * careful.
 *
 * For example, if we accept an incoming connection and only receive
 * some I frames and send no I frames, T1 never gets started.  In my
 * earlier attempt, 't1_remaining_when_last_stopped' had the initial
 * value of 0 lacking any reason to set it differently.  The
 * calculation here then kept pushing t1v up up up.  After receiving
 * 20 I frames and sending none, t1v was over 300 seconds!!!
 *
 * We need some way to indicate that 't1_remaining_when_last_stopped'
 * is not valid and not to use it.  Rather than adding a new variable,
 * it is set to a negative value initially to mean it has not been set
 * yet.  That solves one problem.
 *
 * T1 is paused whenever the channel is busy, either transmitting or
 * receiving, so the measured time could turn out to be a tiny
 * fraction of a second, much less than the frame transmission time.
 * If this gets too low, an unusually long random delay, before the
 * sender's transmission, could exceed this.  I put in a lower limit
 * for t1v, currently 1 second.
 *
 * What happens if we get multiple timeouts because we don't get a
 * response?  For example, when we try to connect to a station which
 * is not there, a KPC-3+ will give up and report failure after 10
 * tries x 4 sec = 40 seconds.
 *
...
void select_t1_value(AX25_data_link_state_machine_t *state) {
    if (state->RC == 0) {
        state->SRTInTicks = (7 * state->SRTInTicks / 8 +
                             AX25_TIMER_T1_PERIOD / 8 -
                             state->T1TimeWhenLastStoppedInTicks / 8);
...
int AX25_MODULO(int vs) {
    // uses masking rather than % so that negative numbers handled correctly
    return (vs & (MODULO - 1));
}
