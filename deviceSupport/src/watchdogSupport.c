...
void StartWatchdogTimer(void)
{
	...
	/* failed to create the RTOS timer */
	ReportError(RTOSfailure, FALSE, ReturnAddr, (int) StartWatchdogTimer);
    }
}
...
{
    /*
     * This reports in all tasks to the watchdogs.  So this would be
     * used after the tasks are going, but when there has been an
     * intentional or expected holdup that might prevent tasks from
     * running.
     */
...
{
    /*
     * This will immediately force a reset on any watchdogs that
     * exist.  So this one is used before all the tasks are running to
     * insure no watchdog this trips.
     */
...
void ReportToWatchdog(WdReporters_t reporter)
{
    ...
    /*
     * If the caller passed CurrentTaskWD, then use that tag
     * associated with the current process.  This could be used in I/O
     * for example.
     */
...
{
    /*
     * The timer calls this one regularly.  The idea here is that if
     * the h/w watchdog is too fast to be practical, we call this fast
     * enough to reset the w/d but then every N times we call it we
     * check if all task have reported in, and if not, do not reset
     * it.
     */
#ifdef WATCHDOG_ENABLE
     ...
     /*
      * We are not going to require that the watchdog be reset by
      * processes as frequently as they need setting.  The external
      * watchdog can't be set for as long as we would like.
      * Therefore, we have the tasks set their bit in a bitmask and
      * every WATCHDOG_CHECK_FREQ_SECS seconds we make sure that they
      * are all set or else we do not reset the watchdog.
      */
	 ...
	 /* Remember we get called twice per second */
	 CheckReportCounter = WD_CHECK_REPORT_FREQ;
     }
     else { // else don't reset the WD so it may die soon!
         printf("WD:%x\n", WatchdogBits);
     }
#else
     // If WD is compiled not enabled, still reset the hardware regularly.
     ResetAllWatchdogs();
#endif
}

#ifdef WD_DEBUG
void StartWatchdogResetDebug(void)
{
    xTimerHandle handle;
    volatile portBASE_TYPE timerStatus;
    int pvtID = 1;
    ...
    /* create a recurring RTOS software timer */
    handle = xTimerCreate( "Debug", CENTISECONDS(100), TRUE, &pvtID,
			   ResetAllWatchdogsStub);
...
/*
 * Use this in place of just plain-old vTaskDelay to ensure there are
 * sufficient calls to the watchdog to keep it happy with your task
 */
