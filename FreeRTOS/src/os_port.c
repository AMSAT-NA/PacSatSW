...
/*
 * This routine used to have a lot of "manual" register setting in the
 * RTI peripheral of the Hercules processor.  I've switched this to using
 * HalCoGen calls so that we can use the RTI for other things as well.
 * I'm not sure where the "User Code Begin" stuff came from.  It looks like
 * they took this out of HalCoGen originally, but then manually changed some
 * stuff.
 */
...
static void prvSetupTimerInterrupt(void)
{
    ...
    /*
     * The work of setting up the counters is currently being done in
     * in rtiInit, called by main.
     *
     */
    /* Enable the compare 0 interrupt.  Note that we don't go through the standard notification
     * routine because vPortPreemptiveTick is commented out in rti.c.  Thus the interrupt comes
     * directly to the vPortPreemptiveTick routine in the OS
     */
