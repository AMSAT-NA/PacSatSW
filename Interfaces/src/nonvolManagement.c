...
#define WRITE_UINT32(field,value) \
    ...
    NVstat = readNV(&states, sizeof(StateSavingMRAM_t), ExternalMRAMData,\
                    (int) &(ptr->StatesInMRAM));\
    if (NVstat == FALSE)\
        ReportError(MRAMread, FALSE, ReturnAddr, (int) WriteMRAMWODFreq);\
    EncodeUint32(value,states.field);\
    NVstat = writeNV(&states, sizeof(StateSavingMRAM_t), ExternalMRAMData,\
                     (int) &(ptr->StatesInMRAM));\
    if (NVstat == FALSE)\
...
bool ReadMRAMBoolState(int index){
    ...
    NVstat = readNV(&safemode, sizeof(StateSavingMRAM_t), ExternalMRAMData,
                    (int) &(ptr->StatesInMRAM));
    if (NVstat == FALSE)
...
void WriteMRAMVersionNumber(void){
    ...
    writeNV(&version, sizeof(uint32_t), ExternalMRAMData,
...
bool CheckMRAMVersionNumber(void){
    ...
    readNV(&version1, sizeof(uint32_t), ExternalMRAMData,
            (int) &(ptr->MRAMVersion1));
    readNV(&version2, sizeof(uint32_t), ExternalMRAMData,
            (int) &(ptr->MRAMVersion2));
...
bool updateCRC(uint32_t address, uint32_t size) {
    ...
    while (w > 0) {
        ...
        NVstat = readNV(&datum, 4, ExternalMRAMData, address);
        if (NVstat == FALSE) {
    ...
    NVstat = writeNV(&result, 4, ExternalMRAMData, address);
    if (NVstat == FALSE) {
