...
bool InitEncryption(void){
	...
	stat = readNV(&magic,sizeof(LocalFlash->AuthenticateKey.magic),ExternalMRAMData,(int)&LocalFlash->AuthenticateKey.magic);
	if(stat){
		stat = readNV(&checksum,sizeof(LocalFlash->AuthenticateKey.keyChecksum),ExternalMRAMData,
	...
	if(stat && (magic == ENCRYPTION_KEY_MAGIC_VALUE)){
		...
		stat = readNV(CKey,sizeof(LocalFlash->AuthenticateKey.key),ExternalMRAMData,(int)&LocalFlash->AuthenticateKey.key);
