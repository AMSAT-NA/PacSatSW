...
/*
 * This module contains routines that talk directly to the TMS570 but
 * which are not generated by or directly related to HalCoGen.
 */
...
{
    /*
     * The error signalling module will call here if we get some
     * group1 errors.  The channel specifies which particular error we
     * have gotten.
     *
     * A couple points here: We want to log most errors, but some
     * require a reboot or even a power cycle.  We will want to ensure
     * that the log is sent to the other processor before we reboot.
     * Although a reboot will normally force nError, that is a switch
     * to the other processor, in these cases we want to switch
     * immediately and then try to send the data over CAN2.
     */
    ...
    case FlashEccCorrectable: {
        /*
         * We keep two data cells for each processor to hold an
         * address with a corrected error.  The data cell number is
         * kept in errNumber and will have the value of 0 and 2; the
         * processor number is in procNum with a value of 0 or 1.  The
         * sum of the two gives us an offset of 0 to 3
         *
         * todo: Swap processors when necessary and set up a timed
         * reboot and force the CAN message to be sent to the other
	...
	//Clear the bits the freeze the corrected address register
        flashWREG->FEDACSTATUS &= 0xFFFFFFF9;
        if(errNumber==0)
	    errNumber=1;
        else
	    errNumber = 0;
    ...
    /*
     * For the even and odd backs of ECC memory, we count them
     * separately, but we use the same pair of 32-bit words to
     * downlink the addresses.  These 32-bit words have the last two
     * addresses that have had a corrected error.
     *
    ...
    case RAMOddBankCorrectableECC: {
	  // g0kla - should this changed from RAMCorAddr1 to
	  // RAMCorAddr2.  TODO - confirm this is correct
        uint32_t *errorLoc = &localErrorCollection.RAMCorAddr1;
    ...
    case DCC1Error: {
        // nError, log, power cycle TODO - does this mean we should
        // actually reboot if this happens
        localErrorCollection.DCC1ErrorCnt++;
        break;
    }

    case N2HET2RamParity: {
        // nError,reboot TODO - does this mean we should actually
        // reboot if this happens
    ...
    case IOMMAccvio: {
        ...
        // Software bug - nERROR, log, reboot - TODO - does this mean
        // we should actually reboot if this happens
        break;
    }
    case DCC2Error: {
	// TODO - surely this should be DCC2ErrorCnt++, updated from
	// DCC1ErrorCnt. g0kla
        localErrorCollection.DCC2ErrorCnt++;
        // nError, log, power cycle - TODO - does this mean we should
        // actually reboot if this happens
    }
    }
...
{
    /*
     * The error signaling module will call here if we get some group2
     * errors.  The channel specifies which particular error we have
     * gotten.
     */
    /*
     * For the moment we log all of these if possible and reboot.  We
     * may want to power cycle, so it will be hard to log.
     */
}
