...
/*
 * I2c bus data are kept in a structure and in arrays indexed by the
 * bus number.  This allows multiple I2c busses to be used.  We only
 * have a single I2c bus on the TMS570, but another one might be added
 * vis the HET
 */
...
{
    /*
     * This routine gets called after the OS starts up to get the OS
     * support part set up, turned on, initialized.  Note that on
     * TMS570, for the hardware I2c (I2C1), the routine i2cInit (the
     * HalCoGen init) must be called first for I2c1 and for the I2c
     * emulator, hetInit (HalCoGen) and HetI2cInit (the emulator code)
     * must be called first.  I did not call these from here because
     * if you call those routines, then you can use I2cSendCommand
     * (below) before the OS is started.
     */
    // Allows us to use this as part of resetting a failed bus
    bool firstTime = firstInit[thisBusNumber];
    ...
    /*
     * Set up the semaphores for use later.  I2cDoneSemaphore is used
     * to make the caller wait until the IO is done.  I2cInUse is a
     * mutex that prevents multiple tasks from using the same I2c bus
     * simultaneously.
     */
    if (firstTime) {
        (void)(vSemaphoreCreateBinary(thisBusData->I2cDoneSemaphore));
	// In use wants a mutex to get priority inheretance
        thisBusData->I2cInUseSemaphore = xSemaphoreCreateMutex();
        if ((thisBusData->I2cDoneSemaphore == NULL)
	        || (thisBusData->I2cInUseSemaphore == NULL)) {
		    ...
		    void *rcvBuffer, uint16_t rcvLength)
{
    ...
    if (!xSemaphoreTake(thisBusData->I2cInUseSemaphore,
